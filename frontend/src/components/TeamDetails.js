...
function TeamDetails() {
  ...
  const handleDeletePlayer = (playerId) => {
   ...
   const handleCardClick = (playerId) => {
     if (updateMode && playerId) {
       navigate(
         `/teams/${teamName}/players/${playerId}`
       );
     }
   };
  ...
  return (
    <div>
      ...
      <div className="flex flex-col bg-[#eeeee4] overflow-y-hidden">
        <div className="bg-[#3A034B] w-full h-64 md:h-64 flex items-center justify-between px-10">
...
{user.admin === 1 && (
          <div className="flex flex-row justify-center space-x-4 mt-[40%] sm:mt-[15%] md:mt-[17%] xl:mt-[10%] md:pb-[5.7rem]">
              ...
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 md:ml-[10%]">
                {players.map((player) => (
                  <div
                    ...
                    className={`
                    relative bg-cover bg-no-repeat rounded-lg flex flex-col items-center h-[320px] w-[230px] sm:h-[300px] sm:w-[200px] md:h-[340px] md:w-[230px] xl:h-[420px] xl:w-[300px] ${updateMode && "cursor-pointer"}`}
                    onClick={() => handleCardClick(player.player_id)}
                    style={{
